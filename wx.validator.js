define(['wx','wx.config','wx.ajax','wx.pop'],function(wx,config){
  ...
  wx.validator = function(context){
    $('form['+prefix+'],form['+prefix+'-ajax]',context||document).each(function(){
        delete wx.validator[$(this).attr('name')];
  ...
  WxForm.prototype.init = function(){
    ...
    this.$submitBn.off('click').click($.proxy(this.checkAll,this));
    $form.off('submit').submit(function(event){
        _this.$submitBn.triggerHandler('click');
        return false;
    });
    if(this.setting.enterSubmit){
      this.$elements.not('[type="checkbox"],[type="radio"]').filter(':visible').filter(':last').off('keypress').keypress(function(event) {
          if(event.keyCode === 13){
            $(this).blur();
  ...
  WxForm.prototype.submit = function(e){
    if(!this.isSubmit){
      this.isSubmit=true;
  ...
  WxForm.prototype.resetSingleError = function($form){
    $(this.setting.singleError,$form).text('');
  }

  WxForm.prototype.singleError = function(errText,$form){
    $(this.setting.singleError,$form).text(errText).show();
  }
  ...
  function Element(ele,wxform){
    ...
    this.setting = {
        ...
        'ae' : $('.'+namePrefix+'error',this.wxform.$form),
        'as' : $('.'+namePrefix+'success',this.wxform.$form)
    };
  }
  ...
  Element.prototype.resetError = function(){
    ...
    $('[class*='+this.setting.nb+']',this.wxform.$form).not('[class='+this.setting.nb+'left]').hide();
    $(this.setting.na+'[class^='+this.setting.nb+']',this.wxform.$form).hide();
    this.setting.ae.hide();
    this.setting.as.hide();
    this.wxform.resetSingleError(this.wxform.$form);
  }
  ...
  Element.prototype.showError = function(errorRule,info){
    ...
    if(this.wxform.setting.singleError){
      this.wxform.singleError(info||errText,this.wxform.$form);
    } else if(this.setting.ae.length){
      this.setting.ae.show().text(info||errText);
    } else if($(this.setting.na+errorRule[0],this.wxform.$form).length){
        $(this.setting.na+errorRule[0],this.wxform.$form).show().text(info);
    } else {
  ...
  Input.prototype.init = function(){
    $.extend(this.setting, {
        ...
        'kb' : this.$ele.attr(prefix+'-keyboard'),
        'ku' : this.$ele.attr(prefix+'-kup'),
    ...
    this.$ele.off(this.setting.et||'blur').on(this.setting.et||'blur',$.proxy(this.valid,this));
  }
  Input.prototype.keyboard = function(){
    var $thisInp = this.$ele,key = this.setting.kb;
    var _this = this;
    $thisInp.off('keypress').keypress(function(event) {
        if(event.keyCode === 13) {
          $(this).blur();
          _this.wxform.$form.submit();
          return false;
        }
    });
    
  }
  Input.prototype.kuvalid = function(){
    var $thisInp = this.$ele,len = this.setting.ku;
    var _this = this;
    $thisInp.off('keyup').keyup(function(){
  ...
  Input.prototype.valid = function(){
    ...
    for(var i=0,l=rules.length;i<l;i++){
        var r = rules[i].split(','),
            ...
            validItem = wx.validator.rule[r[0]].call(this.wxform.$form, value == this.setting.ph?'':value,r[1],this);
  ...
  */
  ...
  wx.validator.rule = {
    ...
    equalTo: function(value, equalToElement) {
      return value.length>0 && value === $("input[name='"+equalToElement+"']",this).val();
    },
